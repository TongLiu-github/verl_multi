...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        if rollout_name == "hf":
            ...
            # 06.08.2025: CHANGE
            # Check if we should use multi-prompt rollout
            use_multi_prompt = self.config.rollout.get("use_multi_prompt", False)

            if use_multi_prompt:
                from verl.workers.rollout.vllm_rollout.vllm_rollout_spmd_multi_prompt import vLLMRollout
                from verl.workers.rollout.vllm_rollout.vllm_rollout_spmd_multi_prompt import vLLMAsyncRollout
            else:
                from verl.workers.rollout.vllm_rollout import vLLMRollout
