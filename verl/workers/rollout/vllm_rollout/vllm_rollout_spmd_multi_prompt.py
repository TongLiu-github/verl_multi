...
class vLLMRollout(BaseRollout):
    def __init__(self, model_path: str, config: DictConfig, tokenizer, model_hf_config, **kwargs):
        """A vLLM rollout with multi-prompt generation capability.
        ...
        super().__init__()
        self.config = config
        self.tokenizer = tokenizer  # Store tokenizer for dynamic prompt generation
        ...
        if max_num_batched_tokens < max_model_len and self.config.enable_chunked_prefill:
            raise ValueError(
                "Enable chunked prefill, max_num_batched_tokens is smaller than max_model_len, \
                             please increase max_num_batched_tokens or disable chunked_prefill"
            )
        ...
        kwargs = dict(
            n=2,  # Each prompt variant generates 2 responses
        ...
        kwargs["n"] = 2  # Each prompt variant generates 2 responses
    ...
    def _generate_prompt_variants(self, question: str) -> List[List[Dict[str, str]]]:
    ...
    def _tokenize_prompt_variants(self, prompt_variants: List[List[Dict[str, str]]]) -> Dict[str, torch.Tensor]:
    ...
    @GPUMemoryLogger(role="vllm rollout spmd multi-prompt", logger=logger)
    @torch.no_grad()
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        """Generate sequences for a batch of prompts with multi-prompt variants.

        Args:
            prompts (DataProto): Input batch.

        Returns:
            DataProto: Output batch with multiple responses per original question.
        """
        # Check if we have original questions for multi-prompt generation
        original_questions = prompts.non_tensor_batch.get("original_questions", None)

        if original_questions is not None:
            # Multi-prompt generation mode
            return self._generate_multi_prompt(prompts, original_questions, **kwargs)
        else:
            # Fallback to original single-prompt generation
            return self._generate_single_prompt(prompts, **kwargs)

    def _generate_multi_prompt(self, prompts: DataProto, original_questions: List[str], **kwargs) -> DataProto:
        """Generate sequences using multiple prompt variants for each question."""

        # Generate prompt variants for each question
        all_prompt_variants = []
        for question in original_questions:
            variants = self._generate_prompt_variants(question)
            all_prompt_variants.extend(variants)

        # Tokenize all prompt variants
        tokenized_variants = self._tokenize_prompt_variants(all_prompt_variants)

        # Create expanded DataProto
        expanded_batch = TensorDict(
            {
                "input_ids": tokenized_variants["input_ids"],
                "attention_mask": tokenized_variants["attention_mask"],
                "position_ids": tokenized_variants["position_ids"],
            },
            batch_size=tokenized_variants["input_ids"].size(0),
        )
        ...
        # Generate using expanded prompts
        return self._generate_single_prompt(expanded_prompts, **kwargs)

    def _generate_single_prompt(self, prompts: DataProto, **kwargs) -> DataProto:
        """Original single-prompt generation logic."""
